...
function switchView(id, url) {
  ...
  if (!views.has(id)) {
    ...
    const timer = setTimeout(() => {
      if (!view._loaded) {
        view._loaded = true
        const errorHtml = `data:text/html,<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{margin:0;display:flex;align-items:center;justify-content:center;height:100vh;background:#0f0f23;color:rgba(255,255,255,.85);font-family:sans-serif;flex-direction:column}.icon{font-size:48px;margin-bottom:16px}.msg{font-size:15px;margin-bottom:8px}.sub{font-size:13px;color:rgba(255,255,255,.4);margin-bottom:24px}.btn{padding:8px 24px;border:1px solid rgba(255,255,255,.2);border-radius:6px;background:transparent;color:rgba(255,255,255,.6);cursor:pointer;font-size:13px;text-decoration:none}.btn:hover{background:rgba(255,255,255,.1)}</style></head><body><div class="icon">⏱️</div><div class="msg">连接超时</div><div class="sub">页面加载超过 15 秒</div><a class="btn" href="${encodeURI(url)}" onclick="event.preventDefault();location.reload()">重新加载</a></body></html>`
        view.webContents.loadURL(errorHtml).catch(() => {})
        // 超时后仍然展示视图（显示错误页）
        if (currentViewId === id) mainWindow.contentView.addChildView(view)
    ...
    view.webContents.on('did-fail-load', (_event, errorCode, errorDesc, validatedURL) => {
      ...
      const msg = errorDesc === 'ERR_NAME_NOT_RESOLVED'
        ...
        ? '连接被拒绝，目标服务器不可达'
        : `加载失败: ${errorDesc}`
      const errorHtml = `data:text/html,<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{margin:0;display:flex;align-items:center;justify-content:center;height:100vh;background:#0f0f23;color:rgba(255,255,255,.85);font-family:sans-serif;flex-direction:column}.icon{font-size:48px;margin-bottom:16px}.msg{font-size:15px;margin-bottom:6px}.sub{font-size:12px;color:rgba(255,255,255,.35);max-width:80%;text-align:center;word-break:break-all;line-height:1.5;margin-bottom:24px}.btn{padding:8px 24px;border:1px solid rgba(255,255,255,.2);border-radius:6px;background:transparent;color:rgba(255,255,255,.6);cursor:pointer;font-size:13px;text-decoration:none}.btn:hover{background:rgba(255,255,255,.1)}</style></head><body><div class="icon">😔</div><div class="msg">连接失败</div><div class="sub">${msg}<br>${displayUrl}</div><a class="btn" href="${encodeURI(displayUrl)}" onclick="event.preventDefault();location.reload()">重新加载</a></body></html>`
      view.webContents.loadURL(errorHtml).catch(() => {})
      // 失败后展示视图（显示错误页）
      if (currentViewId === id) mainWindow.contentView.addChildView(view)
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('config:sync', async () => {
    const config = readConfig()
    const result = await syncFromCloud(config.serverUrl)
    return result
  })
